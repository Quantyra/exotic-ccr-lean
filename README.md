...
| T0.F-branch-density-data | The evaluated algebraic Jacobian determinant has absolute value `2`, its reciprocal factor is `1/2`, and the candidate density `χ(a,c) exp(-τ²)` is defined on branch parameters and extended uniquely over the injective positive branch image (zero off-image) | `ExoticCCR.TheoremFBranchDensity` |
...
- Full unconditional A001 Theorem E remains open in Lean. Incompleteness alone is not used to claim it. The forward-branch modules prove an open positive punctured domain, smooth reconstruction, local injectivity on that positive sheet, norm escape, the algebraic reciprocal Jacobian factor `1/2`, and a pointwise branch-supported candidate `uMinus`. They do not prove a derivative determinant identity, immersion, change-of-variables or measure-`1/2` theorem, ambient `MemLp`, weak-adjoint deficiency, or deficiency indices such as `(∞,∞)`.
