...
- `ForwardSaturatedSheet` is inhabited from every `ForwardBranchOpen` by the explicit map `branchMap (x, sqrt (β x - s))` on `β x - ε₀ < s < β x`. Its upper face escapes and its lower face converges to `qSigma x`. Finite-interval IBP is proved with both traces retained. A separate maximal extension agrees at `qSigma` and has only `-∞`/escape lower ends. Its translated total domain is open, and the translated map is jointly continuous, injective, and an exact right inverse of `F` in `(a,s,c)` coordinates. The measurable/Fubini and change-of-variables infrastructure needed for the global deficiency identity has not yet been proved.
- Full unconditional A001 Theorem E remains open in Lean. Incompleteness alone is not used to claim it. The remaining bridge is measurable change of variables for the translated maximal-sheet image, the `L²` zero extension, endpoint residual convergence under transverse integration, and the global representative-integral identity. Thus the repository still does not prove `X1_forwardWeakDeficiency`, `theoremE`, an adjoint deficiency witness, or deficiency indices such as `(∞,∞)`.
