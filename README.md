...
| T0.F-saturation-infrastructure | Every `ForwardBranchOpen` records the positive half-ball present in its construction. Shrinking it gives a nonempty open transverse set with full vertical segments, fixed-parameter wall escape, and the unconditional identity `tMax(qSigma x)=ε₀`. Connected open partial integral curves glue uniquely to a canonical maximal representative on the interval cut out by `tMin`/`tMax`; every finite endpoint escapes in norm, and each lower endpoint is pointwise either `-∞` or finite and escaping. Along the maximal curve, the middle anchor coordinate has derivative one and equals its initial value plus flow time. A compact transverse closed ball also admits a common positive Picard--Lindelöf lifetime and a jointly continuous local flow. `ForwardSaturatedSheet` still specifies the unproved jointly smooth maximal family and global injectivity data | `ExoticCCR.TheoremFSaturatedSheet`, `ExoticCCR.TheoremFMaximalCoordinate` |
...
- `ForwardSaturatedSheet` is a requirements structure, not an asserted inhabitant. A positive cross-section, a nonempty open vertical collar, and a compact transverse ball with a jointly continuous local flow are constructed. Partial integral curves glue to a canonical pointwise maximal representative on `(tMin,tMax)`, and `d(F₁∘α)/dt=1` identifies its target coordinate with initial `F₁` plus flow time. On the vertical collar, fixed-parameter wall escape and `tMax(qSigma x)=ε₀` are unconditional. Finite lower endpoints escape, giving the required lower-end alternative pointwise. Jointly smooth dependence of the maximal family and the resulting global sheet injectivity remain open, so no `ForwardSaturatedSheet` inhabitant has been assembled.
