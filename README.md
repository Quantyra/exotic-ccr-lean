...
| T0.F-branch-density-data | The Fréchet derivative of real `evalMap F` is the evaluated polynomial Jacobian with determinant `-2`; on the open branch domain its chain derivative equals the target-map derivative. The reciprocal factor is `1/2`. The positive parameter domain and branch image are measurable, the standard-coordinate branch Jacobian is `|τ|`, and mathlib's Jacobian change-of-variables formula is applied to the squared norm of `uMinus` on the image. For continuous compactly supported `χ`, the Gaussian candidate density and its `|τ|`-weighted square are integrable on the relevant parameter domains; a compactly supported cutoff can be chosen so that the measurable zero extension `uMinus` determines a nonzero vector of ambient `L²(ℝ³)`. Along vertical parameter lines inside the open positive domain, its branch pullback has derivative `-2τ uMinus` | `ExoticCCR.TheoremFJacobianFDeriv`, `ExoticCCR.TheoremFBranchDensity` |
| T0.F-saturation-infrastructure | Every `ForwardBranchOpen` contains a smooth injective positive constant-`τ` cross-section at a proved positive offset `ε₀=τ₀²`, with `F(qSigma(a,c))=(a,β(a,c)-ε₀,c)`. A compact transverse closed ball `W₀` inside that cross-section admits a common positive Picard--Lindelöf lifetime for (individually selected) two-sided local `X1` trajectories. Separately, `ForwardSaturatedSheet` specifies the full maximal interval/escape data still required | `ExoticCCR.TheoremFSaturatedSheet` |
...
- `ForwardSaturatedSheet` is a requirements structure, not an asserted inhabitant. A positive cross-section and a compact transverse ball with a uniform local lifetime are constructed, but the trajectories are only selected pointwise: no jointly smooth local flow map, maximal interval, `escape_upper`, or `lower_ok` theorem is proved. The existing `ForwardBranchOpen` is only a local `s = β-τ²` collar: its domain is not proved to contain each full `s`-interval and no lower-end escape/decay theorem is available.
- Full unconditional A001 Theorem E remains open in Lean. Incompleteness alone is not used to claim it. The forward-branch modules prove the local chain identity, the resulting absolute derivative-determinant ratio `1/2` in fixed `Fin 3` coordinates, the exact branch Jacobian `|τ|`, a mathlib Jacobian change-of-variables identity on the branch image, weighted parameter integrability, measurable zero-extension packaging, a nonzero ambient `L²` class represented by a suitable `uMinus`, and its vertical-parameter derivative inside the branch domain. They do not yet prove the global weak-adjoint identity, an adjoint deficiency witness, or deficiency indices such as `(∞,∞)`; the present local open branch is not equipped with a flow-saturated boundary condition needed to discard every integration-by-parts boundary term.
