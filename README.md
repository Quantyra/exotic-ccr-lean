...
- `ForwardSaturatedSheet` is a requirements structure, not an asserted inhabitant. A positive cross-section and pointwise local `X1` trajectories are now constructed, but no uniform product flow collar, maximal interval, or `lower_ok` theorem is proved. The existing `ForwardBranchOpen` is only a local `s = β-τ²` collar: its domain is not proved to contain each full `s`-interval and no lower-end escape/decay theorem is available.
