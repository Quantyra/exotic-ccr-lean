...
- `ForwardSaturatedSheet` is a requirements structure, not an asserted inhabitant. A positive cross-section and a compact transverse ball with a jointly continuous uniform local flow are constructed. Partial integral curves now glue to a canonical pointwise maximal representative on `(tMin,tMax)`, and `d(F₁∘α)/dt=1` identifies its target coordinate with initial `F₁` plus flow time. Finite forward and backward endpoints now provably escape in norm by compact-cluster Picard continuation. Jointly smooth dependence of the maximal family and identification of `tMax(qSigma x)` with the wall offset `ε₀` are not yet proved; consequently no `ForwardSaturatedSheet` inhabitant, `escape_upper` field, or `lower_ok` field has been assembled. The existing `ForwardBranchOpen` remains only a local `s = β-τ²` collar: its exposed API does not prove that each fixed transverse parameter stays in the branch domain all the way to `τ = 0`.
