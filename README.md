...
- `ForwardSaturatedSheet` is inhabited from every `ForwardBranchOpen` by the explicit map `branchMap (x, sqrt (β x - s))` on `β x - ε₀ < s < β x`. Its upper face escapes and its lower face converges to `qSigma x`. Finite-interval IBP is now proved with both traces retained. A separate pointwise maximal extension agrees at `qSigma` and has only `-∞`/escape lower ends. Its selected maximal curves satisfy the cocycle identity and are jointly continuous on a local Picard product collar through every ambient initial point, but continuity on arbitrary compact subdomains of the variable maximal domain is not proved.
- Full unconditional A001 Theorem E remains open in Lean. Incompleteness alone is not used to claim it. The missing bridge is continuation of the local joint flow regularity across compact subdomains of the variable maximal-time bundle, followed by measurable/Fubini or ambient change-of-variables infrastructure and the global representative-integral identity. Thus the repository still does not prove `X1_forwardWeakDeficiency`, `theoremE`, an adjoint deficiency witness, or deficiency indices such as `(∞,∞)`.
