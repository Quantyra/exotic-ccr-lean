...
- `ForwardSaturatedSheet` is inhabited from every `ForwardBranchOpen` by the explicit map `branchMap (x, sqrt (β x - s))` on `β x - ε₀ < s < β x`. Its upper face escapes and its lower face converges to `qSigma x`. Finite-interval IBP is now proved with both traces retained. A separate pointwise maximal extension agrees at `qSigma` and has only `-∞`/escape lower ends, but joint transverse measurability or smoothness of that maximal family is not proved.
- Full unconditional A001 Theorem E remains open in Lean. Incompleteness alone is not used to claim it. The missing bridge is an ambient measurable/change-of-variables treatment of the pointwise maximal sheet (or equivalent parameter-space Fubini infrastructure), followed by the global representative-integral identity. Thus the repository still does not prove `X1_forwardWeakDeficiency`, `theoremE`, an adjoint deficiency witness, or deficiency indices such as `(∞,∞)`.
