...
| T0.F-forward-IBP | On any finite characteristic interval in a `ForwardSaturatedSheet` or `ForwardMaximalSheet`, the density `χ(x) exp(s-β(x))` has derivative equal to itself, the density--test pairing obeys the product rule, and FTC/IBP displays both upper and lower traces explicitly (under the stated interval-integrability hypotheses). On every compact transverse-support box contained in the maximal-sheet domain, continuity gives product integrability and Fubini identifies the product integral with the integrated fiber FTC identity. Separately, `ForwardMaximalSheet` extends each branch collar backward through `qSigma` to `tMin`, proves exact overlap/interface agreement, and leaves only `-∞` or finite escape at the lower maximal end. Its translated variable domain is open; its map is jointly continuous and injective there, and `F(Psi(x,s))=(x₁,s,x₂)`. In standard anchor coordinates the inverse-function theorem gives strict differentiability, constant absolute Jacobian `1/2`, an open measurable image, a measurable embedding, and change-of-variables. The maximal-sheet zero extension is measurable and belongs to ambient `L²` for continuous compactly supported cutoffs whose topological support lies in the transverse open set; a cutoff is constructed for which its `L²` class is nonzero. Endpoint residual convergence and the global representative-integral identity remain open | `ExoticCCR.TheoremEForwardIBP`, `ExoticCCR.TheoremFMaximalSheet`, `ExoticCCR.TheoremFMaximalSheetDensity` |
...
- Full unconditional A001 Theorem E remains open in Lean. Incompleteness alone is not used to claim it. The remaining bridge is transverse Fubini, endpoint residual convergence under transverse integration, and the global representative-integral identity. Thus the repository still does not prove `X1_forwardWeakDeficiency`, `theoremE`, an adjoint deficiency witness, or deficiency indices such as `(∞,∞)`.
