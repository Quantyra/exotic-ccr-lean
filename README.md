...
- `ForwardSaturatedSheet` is a requirements structure, not an asserted inhabitant. A positive cross-section and a compact transverse ball with a jointly continuous uniform local flow are constructed. Reachable-time extrema are definitions, and the proved overlap uniqueness is local; no maximal representative, jointly smooth maximal flow, `escape_upper`, or `lower_ok` theorem is proved. The existing `ForwardBranchOpen` is only a local `s = β-τ²` collar: its domain is not proved to contain each full `s`-interval and no lower-end escape/decay theorem is available.
