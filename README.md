...
- Full unconditional A001 Theorem E remains open in Lean. Incompleteness alone is not used to claim it. The forward-branch module proves an open positive punctured domain, smooth reconstruction, local injectivity on that positive sheet, and norm escape; it does not prove an immersion or measure-`1/2` theorem, a weak-adjoint deficiency vector, `u_-`, or deficiency indices such as `(∞,∞)`.
