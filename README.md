...
| T0.F-saturated-sheet-interface | `ForwardSaturatedSheet` specifies an open nonempty transverse set, a full open interval bundle with an `EReal` lower end, a smooth injective flow parameterization, the exact `F` evaluation and `X1` integral-curve identities, upper escape, and an explicit lower-end alternative (`-∞` or finite escape) | `ExoticCCR.TheoremFSaturatedSheet` |
...
- `ForwardSaturatedSheet` is a requirements structure, not an asserted inhabitant. The existing `ForwardBranchOpen` is only a local `s = β-τ²` collar: its domain is not proved to contain each full `s`-interval and no lower-end escape/decay theorem is available.
