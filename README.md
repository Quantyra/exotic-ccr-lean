...
| T0.F-saturation-infrastructure | Every `ForwardBranchOpen` contains a smooth injective positive constant-`τ` cross-section at a proved positive offset `ε₀=τ₀²`, with `F(qSigma(a,c))=(a,β(a,c)-ε₀,c)`. A compact transverse closed ball `W₀` inside that cross-section admits a common positive Picard--Lindelöf lifetime and a jointly continuous local `X1` flow map on `W₀ × (-δ,δ)`. Connected open partial integral curves glue uniquely to a canonical maximal representative on the interval cut out by the extended-real extrema `tMin`/`tMax`. A Picard continuation argument proves that a finite `tMax` or `tMin` cannot have a compact tail and hence that every finite endpoint escapes in norm. Along the maximal curve, the middle anchor coordinate has derivative one and equals its initial value plus flow time. For a fixed transverse parameter supplied with a full vertical branch segment, the square-root reparameterization is now proved to be an `X1` integral curve on an open interval through time zero; if fixed-parameter escape at `ε₀` is also supplied, its maximal time is proved equal to `ε₀`. Separately, `ForwardSaturatedSheet` specifies the jointly smooth family and wall identification data still required | `ExoticCCR.TheoremFSaturatedSheet`, `ExoticCCR.TheoremFMaximalCoordinate` |
...
- `ForwardSaturatedSheet` is a requirements structure, not an asserted inhabitant. A positive cross-section and a compact transverse ball with a jointly continuous uniform local flow are constructed. Partial integral curves now glue to a canonical pointwise maximal representative on `(tMin,tMax)`, and `d(F₁∘α)/dt=1` identifies its target coordinate with initial `F₁` plus flow time. Finite forward and backward endpoints now provably escape in norm by compact-cluster Picard continuation. The square-root branch is an `X1` curve, and `tMax(qSigma x)=ε₀` follows conditionally when a full fixed-parameter vertical segment and its endpoint escape are supplied. Those hypotheses are not consequences of the current `ForwardBranchOpen`: its exposed API only gives global accumulation at `((0,2),0)`, not a common vertical collar or fixed-parameter escape. Jointly smooth dependence, `lower_ok`, and the missing uniform wall data therefore remain open, and no `ForwardSaturatedSheet` inhabitant has been assembled.
