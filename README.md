...
| T0.F-forward-IBP | On any finite characteristic interval in a `ForwardSaturatedSheet`, the density `χ(x) exp(s-β(x))` has derivative equal to itself, the density--test pairing obeys the product rule, and FTC/IBP displays both upper and lower traces explicitly (under the stated interval-integrability hypotheses). Separately, `ForwardMaximalSheet` extends each branch collar backward through `qSigma` to `tMin`, proves exact overlap/interface agreement, and leaves only `-∞` or finite escape at the lower maximal end. Its translated variable domain is open; its map is jointly continuous and injective there, and `F(Psi(x,s))=(x₁,s,x₂)`. In standard anchor coordinates the inverse-function theorem gives strict differentiability, constant absolute Jacobian `1/2`, an open measurable image, a measurable embedding, and change-of-variables. The maximal-sheet zero extension is measurable and belongs to ambient `L²` for continuous compactly supported cutoffs whose topological support lies in the transverse open set; such nonzero cutoffs exist. Endpoint residual convergence and the global representative-integral identity remain open | `ExoticCCR.TheoremEForwardIBP`, `ExoticCCR.TheoremFMaximalSheet`, `ExoticCCR.TheoremFMaximalSheetDensity` |
...
- `ForwardSaturatedSheet` is inhabited from every `ForwardBranchOpen` by the explicit map `branchMap (x, sqrt (β x - s))` on `β x - ε₀ < s < β x`. Its upper face escapes and its lower face converges to `qSigma x`. Finite-interval IBP is proved with both traces retained. A separate maximal extension agrees at `qSigma` and has only `-∞`/escape lower ends. Its translated total domain is open, and the translated map is jointly continuous, injective, and an exact right inverse of `F` in `(a,s,c)` coordinates. Its standard-coordinate Jacobian is `1/2`, its image is open and measurable, and change of variables is proved. For cutoffs compactly supported inside the transverse open set, the maximal zero extension is measurable and in `L²`; this estimate uses uniform wall-height bounds on compact support rather than regularity of the variable lower endpoint.
- Full unconditional A001 Theorem E remains open in Lean. Incompleteness alone is not used to claim it. The remaining bridge is a nonzero maximal-sheet `L²` witness theorem, endpoint residual convergence under transverse integration, and the global representative-integral identity. Thus the repository still does not prove `X1_forwardWeakDeficiency`, `theoremE`, an adjoint deficiency witness, or deficiency indices such as `(∞,∞)`.
